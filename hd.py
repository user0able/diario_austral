...
def getFecha(
    dias=200,
):  # cantidad de días hacia atrás que buscará el diario :3 / está en 0, porque lo buscará para hoy we :)
...
def genera_url_fecha(fecha=getFecha()):
    url = requests.get(
        "https://www.australvaldivia.cl/impresa/%s/%s/%s/full/cuerpo-principal/1/"
        % (
            fecha.year,
...
for numero in range(1, 50):
    ...
    if requests.get(test_link).status_code == 200:
        ...
        nombre_temporal_archivo = open(
            "temp/metadata_%s" % test_link[-10:], "wb"
        ).write(requests.get(test_link).content)
    else:
